...
/// Number of seconds where, if there's no connections to a Pointer server,
...
/// served the latest version of the Pointer server whenever they load the
/// page. The Pointer server prefixes all assets and connections it loads with
/// its version string, so existing clients can continue to get served even
/// while new clients get new Pointer Server versions.
...
/// Gets the release info from the Pointer path prefix, which is in the
...
impl ConnectionManager {
	...
	/// Gets the StartData for the a version of the Pointer server, triggering
