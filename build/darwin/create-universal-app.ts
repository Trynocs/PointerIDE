...
async function main(buildDir?: string) {
	...
	const x64AppPath = path.join(buildDir, 'Pointer-darwin-x64', appName);
	const arm64AppPath = path.join(buildDir, 'Pointer-darwin-arm64', appName);
	const asarRelativePath = path.join('Contents', 'Resources', 'app', 'node_modules.asar');
	const outAppPath = path.join(buildDir, `Pointer-darwin-${arch}`, appName);
