...
async function main() {
	...
	if (process.env['BUILT_CLIENT']) {
		...
		const clientArchivePath = `.build/win32-${arch}/Pointer-win32-${arch}.zip`;
		await $`7z.exe a -tzip ${clientArchivePath} ../Pointer-win32-${arch}/* "-xr!CodeSignSummary*.md"`.pipe(process.stdout);
