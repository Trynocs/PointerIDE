...
const DISPLAY_NAME = 'Pointer Synced Data';

const MANIFEST_CONTENT = JSON.stringify({
	name: DISPLAY_NAME,
	description: 'Customization data synced from Pointer',
}, null, '\t');
	...
	async bundle(files: readonly ISyncableFile[]): Promise<IBundleResult | undefined> {
		...
		return {
			ref: {
				...
				description: `${syncable.length} customization(s) synced from Pointer`,
				nonce,
			},
		};
	}
