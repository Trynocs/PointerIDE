...
The management editor lives in `vs/workbench` (shared between core Pointer and sessions):
...
├── aiCustomizationWorkspaceService.ts          # Core Pointer workspace service impl
...
| Property / Method | Core Pointer | Agent Sessions Window |
|----------|-------------|----------|
...
- **Factory functions** — `createVSCodeHarnessDescriptor`, `createCliHarnessDescriptor`, `createClaudeHarnessDescriptor`. The Pointer harness receives `[PromptsStorage.extension, BUILTIN_STORAGE]` as extras; CLI and Claude in core receive `[]` (no extension source). Sessions CLI receives `[BUILTIN_STORAGE]`.
...
In core Pointer, all three harnesses are registered but CLI and Claude only appear when their respective agents are registered (`requiredAgentId` checked via `IChatAgentService`). Pointer is the default.
...
### Built-in Extension Grouping (Core Pointer)

In core Pointer, customization items contributed by the default chat extension (`productService.defaultChatAgent.chatExtensionId`, typically `GitHub.copilot-chat`) are grouped under the "Built-in" header in the management editor list widget, separate from third-party "Extensions".
...
- **Creation targets**: `getSourceFolders()` override replaces Pointer profile user roots with `~/.copilot/{subfolder}` for CLI compatibility
