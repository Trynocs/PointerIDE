...
6. Confirm the path is NOT the Pointer profile folder (e.g., NOT `~/.vscode-oss-sessions-dev/User/...`)
...
- User file is created under `~/.copilot/instructions/` (not the Pointer profile folder)
...
- The Pointer profile folder should never be used for user creation in sessions
