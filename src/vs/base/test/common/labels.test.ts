...
suite('Labels', () => {
	...
	test('template', () => {
		...
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: '', appName: 'Pointer', separator: { label: ' - ' } }), 'Pointer');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: 'Untitled-1', rootName: '', appName: 'Pointer', separator: { label: ' - ' } }), 'Untitled-1 - Pointer');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: 'monaco', appName: 'Pointer', separator: { label: ' - ' } }), 'monaco - Pointer');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: 'somefile.txt', rootName: 'monaco', appName: 'Pointer', separator: { label: ' - ' } }), 'somefile.txt - monaco - Pointer');
		assert.strictEqual(labels.template(t, { dirty: '* ', activeEditorShort: 'somefile.txt', rootName: 'monaco', appName: 'Pointer', separator: { label: ' - ' } }), '* somefile.txt - monaco - Pointer');

		// real world example (other)
		t = '${dirty}${activeEditorShort}${separator}${rootNameShort}${separator}${appName}';
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: 'monaco (Workspace)', rootNameShort: 'monaco', appName: 'Pointer', separator: { label: ' - ' } }), 'monaco - Pointer');
	});
