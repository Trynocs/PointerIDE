# Pointer Development MCP Server

This directory contains a Model Context Protocol (MCP) server that provides Pointer automation capabilities for Pointer development and testing. The MCP server exposes Pointer's testing infrastructure through a standardized interface, allowing AI assistants and other tools to interact with Pointer programmatically.

## What is MCP?

The [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) is an open standard that enables AI assistants to securely connect to external data sources and tools. This MCP server specifically provides Pointer automation capabilities, making it possible for AI assistants to:

- Start and stop Pointer instances
...
The MCP server exposes a comprehensive set of Pointer automation tools through the MCP protocol:

### Application Management
- Start, stop, and restart Pointer instances
...
│   ├── application.ts       # Pointer application lifecycle management
...
- **application.ts** - Manages Pointer application lifecycle (start, stop, restart)
- **automationTools/** - Modular tool implementations organized by Pointer feature area

## Troubleshooting

### Server Won't Start
- Ensure Pointer has been built and run at least once (via F5 or `code.sh`)
...
This MCP server is part of the Pointer development infrastructure. When making changes:
