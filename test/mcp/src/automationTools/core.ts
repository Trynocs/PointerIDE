...
export function applyCoreTools(server: McpServer, appService: ApplicationService): RegisteredTool[] {
	...
	tools.push(server.tool(
		...
		'Restart Pointer with optional workspace or folder and extra command-line arguments',
		{
		...
		async ({ workspaceOrFolder, extraArgs }) => {
			...
			return {
				content: [{
					type: 'text' as const,
					text: `Pointer restarted successfully${workspaceText}${argsText}`
				}]
			};
		}
	));
	...
	tools.push(server.tool(
		...
		async () => {
			...
			return {
				content: [{
					type: 'text' as const,
					text: 'Pointer stopped successfully'
				}]
			};
		}
	));
	...
	// 	'Get the quality/build type of Pointer (Dev, Insiders, Stable, etc.)',
	// 	async () => {
	// 		const info = {
	// 			quality: app.quality,
	// 			remote: app.remote,
	...
	// 	'Send a keybinding to Pointer (e.g., ctrl+shift+p, cmd+s)',
	// 	{
