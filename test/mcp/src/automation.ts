...
export async function getServer(appService: ApplicationService): Promise<Server> {
	const server = new McpServer({
		name: 'Pointer Automation Server',
		version: '1.0.0',
		title: 'An MCP Server that can interact with a local build of Pointer. Used for verifying UI behavior.'
	}, { capabilities: { logging: {} } });

	server.tool(
		'vscode_automation_start',
		'Start Pointer Build. If workspacePath is not provided, Pointer will open with the last used workspace or an empty window.',
		{
		...
		async ({ recordVideo, workspacePath }) => {
			...
			return {
				content: [{
					type: 'text' as const,
					text: app ? `Pointer started successfully${workspacePath ? ` with workspace: ${workspacePath}` : ''}` : `Failed to start Pointer`
				}]
			};
		}
	);

	// Apply all Pointer automation tools using the modular structure
