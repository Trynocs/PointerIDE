...
class SetupAgent extends Disposable implements IChatAgentImplementation {
	...
	static registerVSCodeAgent(instantiationService: IInstantiationService, context: ChatEntitlementContext, controller: Lazy<ChatSetupController>): { agent: SetupAgent; disposable: IDisposable } {
		return instantiationService.invokeFunction(accessor => {
			...
			const { agent, disposable } = SetupAgent.doRegisterAgent(instantiationService, chatAgentService, 'setup.vscode', 'vscode', false, localize2('vscodeAgentDescription', "Ask questions about Pointer").value, ChatAgentLocation.Panel, undefined, context, controller);
			...
			disposables.add(SetupTool.registerTool(instantiationService, {
				...
				modelDescription: localize('setupToolsDescription', "Scaffold a new workspace in Pointer"),
				userDescription: localize('setupToolsDescription', "Scaffold a new workspace in Pointer"),
