...
/**
 *
 * We keep parsing in two phases to mirror Pointer's shape:
 ...
 * workspace root. Pointer's built-in detector cannot resolve that context.
 */
export class CopilotCLITerminalLinkProvider implements TerminalLinkProvider<CopilotCLITerminalLink> {
	...
	async provideTerminalLinks(context: TerminalLinkContext, token: CancellationToken): Promise<CopilotCLITerminalLink[]> {
		...
		// Match Pointer's built-in MaxLineLength limit (terminalLocalLinkDetector.ts).
		...
		for (const candidate of this._detectLinkCandidates(line)) {
			// Match Pointer's built-in MaxResolvedLinksInLine (terminalLocalLinkDetector.ts).
			...
			// Mirrors Pointer's specialEndCharRegex (terminalLocalLinkDetector.ts).
			let trimmed = 0;
