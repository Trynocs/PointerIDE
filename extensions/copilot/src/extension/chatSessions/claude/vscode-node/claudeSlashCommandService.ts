...
export interface IClaudeSlashCommandService {
	 ...
	 *
	 * Checks `request.command` first (Pointer slash command), then falls back to
...
export class ClaudeSlashCommandService extends Disposable implements IClaudeSlashCommandService {
	...
	constructor(
		...
		super();
		// Initialize eagerly to register Pointer commands at startup
	...
	async tryHandleCommand(
		...
		// 1. Check request.command (Pointer slash command selected via UI)
		if (request.command) {
	...
	private _ensureInitialized(): void {
		...
		for (const ctor of ctors) {
			...
			// Register Pointer command if commandId is provided
