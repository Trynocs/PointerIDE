...
/**
 * Copies the node-pty files from Pointer's installation into a @github/copilot location.
 *
 ...
 * Code's app tree). For built-in installs `require('node-pty')` walks up into Pointer's
 * own `node_modules` and this shim is a no-op (the `shims.txt` placeholder skips it). For
 * marketplace installs we copy Pointer's pre-built `pty.node` into the SDK's expected
 ...
 * @param vscodeAppRoot Pointer's installation path (where node-pty is located)
 */
