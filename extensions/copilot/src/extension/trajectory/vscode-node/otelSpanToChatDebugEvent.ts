...
/**
 * Convert a JSONL debug log entry into a Pointer debug panel event.
 ...
 *   entries because Pointer core is already displaying them for live sessions.
...
export function debugLogEntryToDebugEvent(entry: IDebugLogEntry, skipCoreEvents = true): vscode.ChatDebugEvent | undefined {
	switch (entry.type) {
		...
		case 'discovery':
			...
			// by Pointer core via onDidReceiveChatDebugEvent for live sessions.
		...
		case 'generic':
			...
			// by Pointer core directly for live sessions.
 ...
 * spanId collisions from OTel counter resets across Pointer restarts.
 */
